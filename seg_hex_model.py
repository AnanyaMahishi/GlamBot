...
image = facer.hwc2bchw(facer.read_hwc('C:/Users/Ananya Mahishi/Github/glambot/glambot_app/uploads/celeb.jpg')
...
facial_parts = [
    "Background", "Neck" ,"Skin","Cloth" , "Right Ear", "Left Ear", "Right Brow","Left Brow",   "Right Eye","Left Eye","Nose","Mouth",  "Lower Lip","Upper Lip", "Hair","Eye Glasses",
...
for class_idx in range(n_classes):
    ...
    segment_image.save(f'segment_{class_idx}_{part_name}.png')

    # Add the segment image to the list
    all_segments.append((class_idx, segment_image, part_name))

# Display all segments
n_all_segments = len(all_segments)
fig, axs = plt.subplots(1, n_all_segments, figsize=(5 * n_all_segments, 5))  # Adjust figsize as needed

for idx, (class_idx, segment_image, part_name) in enumerate(all_segments):
...
for class_idx in range(n_classes):
    ...
    if class_mask.sum() > 0:
        ...
        segment_image = Image.fromarray(segment_image.astype('uint8'))
        part_name = facial_parts[class_idx] if class_idx < len(facial_parts) else f'Class_{class_idx}'
        segment_image.save(f'segment_{part_name}.png')
